...
4. **Surplus** — A pure dashboard over the Cash Flow inputs. Three boxes: Income (gross and net, regular-only vs. bonus-adjusted), Monthly Budget (housing, bills, retirement, benefits, tithe, taxes), and Surplus (annual/quarterly/monthly, plus the regular-vs-bonus split). Allocation is split into two parallel blocks — regular pay allocated monthly, bonus allocated annually — each dividing across discretionary, home savings, car savings, kids' future, and house payoff, with house payoff as the auto-set remainder.
